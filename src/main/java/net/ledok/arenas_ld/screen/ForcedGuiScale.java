 ...
 *
 * <p>The screens are authored at a fixed design resolution. Rather than matrix-scaling the
 * rendered UI by a fractional factor (which makes Minecraft's bitmap font blurry and shaves edge
 * pixels), this forces the window's GUI scale to an integer for the lifetime of the screen so the
 * font renders natively and crisply. The largest integer scale whose layout fits the window is
 * used (capped at {@link #MAX_SCALE}), so the screen fills as much of the window as it can while
 * staying sharp — Scale 2 on a 1080p window, more on higher-resolution displays, stepping down to
 * Scale 1 only on small windows.</p>
 *
